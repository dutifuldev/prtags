...
## Horse Variation Enhancement

This Horse variation of JSend extends the original specification to allow for optional `message` fields in all response types, providing consistent messaging capabilities across success, fail, and error responses.

<table>
<tr><th>Type</td><th>Description</th><th>Required Keys</th><th>Optional Keys</td></tr>
<tr><td>success</td><td>All went well, and (usually) some data was returned.</td><td>status, data</td><td>message</td></tr>
...
```
#### DELETE /posts/2.json (with message): ####
```
{
    status : "success",
    data : null,
...
* data: Acts as the wrapper for any data returned by the API call. If the call returns no data, data should be set to null.

Optional keys:

* message: A human-readable message providing additional context about the successful operation (Horse variation extension).
...
* message: A human-readable message providing additional context about the failure (Horse variation extension).
